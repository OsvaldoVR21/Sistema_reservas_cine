...
class Empleado(Persona):
    ...
    def gestionar_funciones(self):
        if self.rol == RolEmpleado.ADMIN:
            print(f"Acceso concedido: El administrador {self.nombre} esta gestionando las funciones")
            return True
        else:
...
##Pelicula
class Pelicula:
    def __init__(self, titulo, duaracion, clasificacion, genero):
        self.titulo = titulo
        self.duaracion = duaracion
...
class Reserva:
    def __init__(self, id_reserva: int, usuario: Usuario, funcion: Funcion, asientos: list):
        for asiento in asientos:
            if asiento in funcion.sala.asientos_ocupados:
                raise ValueError(f"Error: El asiento {asiento}ya esta ocupado")
    ...
    def aplicar_promocion(self, promo: Promocion):
        if promo.es_valida():
            ...
            print(f"Promoción {promo.codigo} aplicada. Nuevo monto: {self.monto_total}")
        else:
            print(f"Promoción {promo.codigo} no es válida")
    ...
    def generar_ticket(self):
        print("\n"+"="*30)
        print("CINEMAS PROGRAAVANZADA")
        print("\n"+"="*30)
        print(F"TICKET DE RESERVA #{self.id_reserva}")
        print("\n"+"-"*30)
